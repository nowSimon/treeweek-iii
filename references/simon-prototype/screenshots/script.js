...
gsap.set('#envelope-wrap', { scale: 3.2, y: -80, opacity: 0, rotation: 0 });
...
const letterContinued = document.getElementById('letter-continued');
const hexagramEl = document.getElementById('hexagram');
const readingEl = document.getElementById('reading');

gsap.set(letterPaper, { yPercent: 55 });

function positionBubble(bubble, anchorEl, verticalBias) {
  const paperRect = letterPaper.getBoundingClientRect();
  const anchorRect = anchorEl.getBoundingClientRect();
  const anchorMidY = anchorRect.top - paperRect.top + anchorRect.height / 2;
  const pointerReach = 38; // distance from the bubble's bottom edge to the tip of its trailing dot (see .thought-bubble::before)
...
const topLetterImg = document.querySelector('#letter-scene .letter-photo-wrap:not(.letter-signoff-wrap) .letter-photo');
whenImageReady(topLetterImg, () => positionBubble(fateBubble, fateLine, -20));
window.addEventListener('resize', () => positionBubble(fateBubble, fateLine, -20));

if (window.ResizeObserver) {
  new ResizeObserver(() => positionBubble(fateBubble, fateLine, -20)).observe(letterPaper);
}

seal.addEventListener('click', () => {
  gsap.set(letterPaper, { zIndex: 10 });
  gsap.to(letterPaper, {
    yPercent: 0,
    opacity: 1,
    visibility: 'visible',
  ...
  setTimeout(() => positionBubble(fateBubble, fateLine, -20), 600);
  setTimeout(() => fateBubble.classList.add('shown'), 10000);
});
...
fateBubble.addEventListener('click', () => {
  ...
  letterContinued.hidden = false;
  gsap.set(letterContinued, { height: 0, opacity: 0 });
  gsap.to(letterContinued, {
    height: 'auto',
    opacity: 1,
    duration: 0.8,
    ease: 'power2.out',
    onComplete: () => {
      const bottomLetterImg = document.querySelector('.letter-signoff-wrap .letter-photo');
      whenImageReady(bottomLetterImg, () => {
        requestAnimationFrame(() => requestAnimationFrame(() => positionBubble(acceptBubble, signoff, -20)));
      });
      setTimeout(() => acceptBubble.classList.add('shown'), 10000);
    },
  });
});
