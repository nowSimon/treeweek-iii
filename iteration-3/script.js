...
gsap.set('#envelope-wrap', { scale: 1.15, y: -80, opacity: 0, rotation: 0 });
...
gsap.set(letterPaper, { xPercent: -50, yPercent: 55 });
...
function placeFateBubble() {
  positionBubble(fateBubble, fateLine, -20);
}

placeFateBubble();
document.fonts.ready.then(placeFateBubble);
window.addEventListener('resize', placeFateBubble);

if (window.ResizeObserver) {
  new ResizeObserver(placeFateBubble).observe(letterPaper);
}
  ...
  const tl = gsap.timeline({
    onComplete: () => {
      requestAnimationFrame(() => {
        ...
        gsap.delayedCall(0.6, () => acceptBubble.classList.add('shown'));
      });
    },
  });

  tl.from(letterContinued, { opacity: 0, duration: 0.4, ease: 'power2.out' })
    .from('.coin', {
      y: -60,
      opacity: 0,
